...
class HelpScene: SKScene {
    ...
    func loadBackButtonTextures() {
        let buttonAtlas = SKTextureAtlas(named: "BackButtonAnimations")
        
        for i in 1...1 {
            let textureName = "backButton\(i)"
            let temp = buttonAtlas.textureNamed(textureName)
            self.backButtonTextures.append(temp)
        }
    }
    func loadBackButtonTexturesIphone6Plus() {
        let buttonAtlas = SKTextureAtlas(named: "BackButtonAnimationsIphone6Plus")
        
        for i in 1...1 {
            let textureName = "backButt\(i)"
            let temp = buttonAtlas.textureNamed(textureName)
